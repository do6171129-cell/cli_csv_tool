...
プレビュー表示（先頭 N 件）のみ、表示整形を行っています。

- 列幅を自動計算して揃える
- 数値は右寄せ、それ以外は左寄せ
- 欠損値（`None` / 空文字 / キー無し）は `"-"` として表示
- 表示整形は表示専用であり、行データ（rows）は一切変更しない

表示整形は `view.py` の `render_preview()` に集約しています。

※ 教育的判断により、  
　件数表示・列一覧表示など他の出力は簡易表示のままとしています。
